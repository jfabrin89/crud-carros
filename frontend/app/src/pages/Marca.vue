            ...
            <v-toolbar flat color="white">
              ...
              <v-dialog v-model="dialog" max-width="500px">
                <template v-slot:activator="{ on }">
                  <v-btn color="primary" dark class="mb-2" v-on="on">Nova Marca</v-btn>
                </template>
                <v-card>
                  <v-card-title>
  ...
  methods: {
    ...
    async save () {
      if (this.editedIndex > -1) {
        await this
          .axios
          .put('marca/' + this.editedItem.id, this.editedItem)
          .then(response => {
